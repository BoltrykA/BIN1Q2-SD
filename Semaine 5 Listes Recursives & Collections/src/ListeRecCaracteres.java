...
public class ListeRecCaracteres {
	...
	private void remplacerTout(NoeudCaractere noeud, char caractereARemplacer, char nouveauCaractere) {
		if (noeud == null) return;
		if (noeud.caractere == caractereARemplacer) noeud.caractere = nouveauCaractere;
	...
	public char max() throws NoSuchElementException{
		if (tete == null) throw new NoSuchElementException("liste vide");
		return max(tete);
	...
	private char max(NoeudCaractere noeud) {
		if (noeud == null) return ' ';
		char maxChar = max(noeud.suivant);
		if (noeud.caractere > maxChar) return noeud.caractere;
		return maxChar;
	}
	...
	private boolean estEgalA(NoeudCaractere noeud, NoeudCaractere l) {
		if (noeud == null && l == null) return true;
		if (noeud == null || l == null) return false;
		if (noeud.caractere != l.caractere) return false;
		return estEgalA(noeud.suivant, l.suivant);
	}
