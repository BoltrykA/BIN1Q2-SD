...
public class ListeSimpleImpl<E> implements ListeSimple<E> {
	...
	// insere un nouveau noeud apres le noeud contenant la premiere occurrence de l'element passe en parametre
	public boolean insererApres(E element, E elementAInserer){
		Noeud baladeur = tete;
		while (baladeur != null){
			if (baladeur.element.equals(element)){
				Noeud node = new Noeud(elementAInserer,baladeur.suivant);
				baladeur.suivant = node;
				taille++;
				return true;
			}
			baladeur = baladeur.suivant;
