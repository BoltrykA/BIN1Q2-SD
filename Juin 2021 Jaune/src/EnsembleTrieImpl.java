...
public class EnsembleTrieImpl<E> implements EnsembleTrie<E>{
	...
	public E elementLePlusGrand() {
		if (racine == null || taille == 0) return null;
		//Lisez bien les explications pour l'utilisation de la methode compareTo() dans l'enonce
		return elementLePlusGrand(racine.droit, racine.element);
	}

	private E elementLePlusGrand(Noeud noeud, E element){
		if (noeud == null) return element;
		if (((Comparable<E>) noeud.element).compareTo(element) >= 0){
			return elementLePlusGrand(noeud.droit, noeud.element);
		}
		return elementLePlusGrand(noeud.droit, element);
	}
	...
	private boolean ajouter(Noeud noeud, E element){
		...
		if (((Comparable<E>) noeud.element).compareTo(element) == 0) return false;
	...
	public E elementSuivant(E element) {
		...
		if (i == -1 || (i+1) == liste.size()) return null;
