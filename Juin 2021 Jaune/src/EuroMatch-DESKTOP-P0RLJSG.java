...
public class EuroMatch {
	...
	public boolean placerDansFileAttente(String client){
		if (client == null || client.equals("") || nombrePlacesRestantes == 0) return false;
		...
		if (reservation != null && reservation.getNombrePlacesReservees() == MAX_PLACES_CLIENT) return false;
	...
	public String selectionnerClientSuivant(){
		if (ensembleClientsActuellementDansFile.isEmpty()) return null;
	...
	public boolean passerNouvelleReservation(String client, int nombrePlacesDemandees){
		...
		if (nombrePlacesDemandees <= 0) throw new IllegalArgumentException();
		if (mapClientReservation.containsKey(client)) throw new IllegalStateException();
		if (nombrePlacesDemandees > nombrePlacesRestantes || nombrePlacesDemandees > MAX_PLACES_CLIENT) return false;
		Reservation reservation = new Reservation(client,nombrePlacesDemandees);
	...
	public boolean modifierReservation(String client,int nombrePlacesDemandeesEnPlus){
		if (!mapClientReservation.containsKey(client)) throw new IllegalStateException();
		if (client == null || client.equals("") || nombrePlacesDemandeesEnPlus <= 0) throw new IllegalArgumentException();
		Reservation reservation = mapClientReservation.get(client);
		int nb = reservation.getNombrePlacesReservees()+nombrePlacesDemandeesEnPlus;
		if (nb > MAX_PLACES_CLIENT || nb > nombrePlacesRestantes) return false;
		reservation.setNombrePlacesReservees(nb);
